    ...
    setConfig: (config: Protobuf.Config.Config) => {
      set(
        produce<PrivateDeviceState>((draft) => {
          const device = draft.devices.get(id);
          if (device) {
            console.log(`[DeviceStore] setConfig called for device ${id}. Variant: ${config.payloadVariant.case}`);
            switch (config.payloadVariant.case) {
              ...
              case "lora": {
                console.log(`[DeviceStore] Setting lora config for device ${id}`, config.payloadVariant.value);
            ...
            // Force config reference update to ensure React/Zustand detects the change
            // This handles cases where Immer might not track mutations inside Protobuf message instances cleanly
            device.config = { ...device.config };
            console.log(`[DeviceStore] device.config keys AFTER update: ${Object.keys(device.config)}`);
            if ('lora' in device.config) {
               console.log(`[DeviceStore] device.config.lora is PRESENT:`, device.config.lora);
            } else {
               console.error(`[DeviceStore] device.config.lora is MISSING after set!`);
            }
          } else {
             console.warn(`[DeviceStore] setConfig: Device ${id} not found!`);
          }
        }),
      );
    },
